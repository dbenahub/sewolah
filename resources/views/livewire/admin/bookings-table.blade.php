        ...
        <tbody>
          @foreach($leads as $lead)
            <tr class="border-t border-white/6">
              ...
              <td class="p-3.5">
                <select wire:change="updateStatus({{ $lead->id }}, $event.target.value)" class="bg-transparent border border-white/15 rounded px-2 py-1 text-xs">
                  @foreach(['baru','dihubungi','quotation_dihantar','disahkan','batal'] as $s)
                    <option value="{{ $s }}" @selected($lead->status === $s)>{{ ucfirst(str_replace('_',' ',$s)) }}</option>
                  @endforeach
                </select>
              </td>
              <td class="p-3.5">
